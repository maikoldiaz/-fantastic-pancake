// --------------------------------------------------------------------------------------------------------------------
// <copyright file="SaveIntegrationDeltaFile.cs" company="Microsoft">
...
namespace Ecp.True.Processors.Delta.Integration
{
    using System;
    ...
    /// <summary>
    /// The delta integration with FICO.
    /// </summary>
    public class SaveIntegrationDeltaFile : ISaveIntegrationDeltaFile
    {
        /// <summary>
        ...
        public async Task<string> RegisterIntegrationAsync(IntegrationData integrationData)
        {
            ...
            var typeFico = TypeNameFolderFico(integrationData.Type);
            integrationData.BlobPath = $"{typeFico}/{integrationData.BlobName}";

            await this.SaveDeltaToBlobAsync(integrationData.Data, integrationData.BlobPath).ConfigureAwait(false);
        ...
        /// <summary>
        /// Types the name folder fico.
        /// This method is for renaming "operativeDelta" in "delta".
        /// </summary>
        /// <param name="integrationData">The integration data.</param>
        /// <returns>Name route.</returns>
        private static string TypeNameFolderFico(SystemType integrationData)
        {
            return integrationData == SystemType.OPERATIVEDELTA ? "delta" : integrationData.ToString().ToLowerCase();
        }

        /// <summary>
        /// Saves the delta to BLOB asynchronous.
        /// </summary>
        /// <param name="data">The data.</param>
        /// <param name="blobName">Name of the BLOB.</param>
        /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
        private Task SaveDeltaToBlobAsync(string data, string blobName)
        {
            return this.azureClientFactory.GetBlobStorageSaSClient(ContainerName.Delta, blobName).CreateBlobAsync(data);
        }
