...
-- <Description>:   This procedure is used to get the Graphical Destination Node details for a given Source Node. </Description>
EXEC [Admin].[usp_GetGraphicalDestinationNodesDetails] 2200
-- ==============================================================================================================================*/
CREATE PROCEDURE [Admin].[usp_GetGraphicalDestinationNodesDetails]
(
	     @NodeId	    INT
)
AS
BEGIN
	...
	-- NODETAG CATEGORY ELEMENT TEMP TABLE
	   ...
	   WHERE (NT.NodeId IN (SELECT DestinationNodeId 
     ...
     -- GET NODE DETAILS
        ...
        SELECT          ND.NodeId
					   ...
					   ,ND.[Order]
           FROM [Admin].[Node] ND
		   WHERE (ND.NodeId IN (SELECT DestinationNodeId 
		                       FROM [Admin].[NodeConnection] 
		                      WHERE SourceNodeId = @NodeId
							    AND IsDeleted = 0
							)
...
EXEC sp_addextendedproperty @name = N'MS_Description',
    @value = N'This procedure is used to get the Graphical Destination Node details for a given Source Node.',
    @level0type = N'SCHEMA',
    @level0name = N'Admin',
    @level1type = N'PROCEDURE',
    @level1name = N'usp_GetGraphicalDestinationNodesDetails',
    @level2type = NULL,
    @level2name = NULL