...
WORKDIR /src
COPY ["Ecp.True/Ecp.True.Host.Api/Ecp.True.Host.Api.csproj", "Ecp.True/Ecp.True.Host.Api/"]
...
RUN dotnet restore "Ecp.True/Ecp.True.Host.Api/Ecp.True.Host.Api.csproj"
COPY . .
WORKDIR "/src/Ecp.True/Ecp.True.Host.Api"
RUN dotnet build "Ecp.True.Host.Api.csproj" -c Release -o /app

FROM build AS publish
RUN dotnet publish "Ecp.True.Host.Api.csproj" -c Release -o /app

FROM base AS final
WORKDIR /app
COPY --from=publish /app .
ENTRYPOINT ["dotnet", "Ecp.True.Host.Api.dll"]