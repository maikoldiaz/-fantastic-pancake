...
function mountWithRealStore() {
    ...
    const props = {
        ...
        ownersToggler: data.nodeConnection.attributes.ownersToggler,
        owners: data.nodeConnection.attributes.owners,
...
describe('OwnersSelect', () => {
    ...
    it('should call onClose on click of cancel button', () => {
        const { enzymeWrapper } = mountWithRealStore();
        enzymeWrapper.find(ModalFooter).find('#btn_nodeProducts_ownersselect_cancel').simulate('click');
        expect(enzymeWrapper).toHaveLength(1);
    });

    it('should call on wizard next on click of next button', () => {
        const { enzymeWrapper } = mountWithRealStore();
        enzymeWrapper.find(ModalFooter).find('#btn_nodeProducts_ownersselect_submit').at(0).simulate('click');
        expect(enzymeWrapper).toHaveLength(1);
    });

    it('ownersToggler update should toggle the property', () => {
        const { props } = mountWithRealStore();
        props.ownersToggler = true;
