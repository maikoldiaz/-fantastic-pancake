...
import OwnersSelect, { OwnersSelect as OwnersSelectComponent } from '../../../../../modules/administration/nodeConnection/attributes/components/ownersSelect.jsx';
...
describe('OwnersSelect', () => {
    ...
    it('should have cancel and next button', () => {
        const { enzymeWrapper } = mountWithRealStore();
        expect(enzymeWrapper).toHaveLength(1);
        expect(enzymeWrapper.find(ModalFooter).find('#btn_connectionProducts_owners_select_cancel').text()).toEqual('cancel');
        expect(enzymeWrapper.find(ModalFooter).find('#btn_connectionProducts_owners_select_submit').text()).toEqual('next');
    });

    it('should call onClose on click of cancel button', () => {
        const { enzymeWrapper, props } = mountWithRealStore();
        enzymeWrapper.find(ModalFooter).find('#btn_connectionProducts_owners_select_cancel').simulate('click');
    });
