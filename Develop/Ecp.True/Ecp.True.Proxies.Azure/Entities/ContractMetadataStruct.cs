...
namespace Ecp.True.Proxies.Azure.Entities
{
    ...
    public class ContractMetadataStruct
    {
        ...
        {
            return $"{this.ContractName} : Contract Address first 8 bytes: {this.ContractAddress?.Substring(0, 8)} " +
                $": Contract ABI first 8 bytes: {this.ContractAbi?.Substring(0, 8)} : {this.ContractVersion}";
        }
    }
}
