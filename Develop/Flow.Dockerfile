...
WORKDIR /src
COPY ["Ecp.True/Ecp.True.Host.Flow.Api/Ecp.True.Host.Flow.Api.csproj", "Ecp.True/Ecp.True.Host.Flow.Api/"]
COPY ["Ecp.True/Ecp.True.Processors.Approval/Ecp.True.Processors.Approval.csproj", "Ecp.True/Ecp.True.Processors.Approval/"]
...
RUN dotnet restore "Ecp.True/Ecp.True.Host.Flow.Api/Ecp.True.Host.Flow.Api.csproj"
COPY . .
WORKDIR "/src/Ecp.True/Ecp.True.Host.Flow.Api"
RUN dotnet build "Ecp.True.Host.Flow.Api.csproj" -c Release -o /app

FROM build AS publish
RUN dotnet publish "Ecp.True.Host.Flow.Api.csproj" -c Release -o /app

FROM base AS final
WORKDIR /app
COPY --from=publish /app .
ENTRYPOINT ["dotnet", "Ecp.True.Host.Flow.Api.dll"]