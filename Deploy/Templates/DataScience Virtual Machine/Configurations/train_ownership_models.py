...
Script creado para el entrenamiento y prueba de modelos analíticos 
...
def prophet_evaluation_table(analytic_database,periods):
    ...
    test_data =  analytic_database_.iloc[-periods:,:]
    #Model training
    prophet_model = Prophet(interval_width=0.90,daily_seasonality =False,weekly_seasonality=False) #Set parameters of the model
...
def features_selection(analytic_database,pop_size,mutation_prob,elite_perc,pop_perc,generations,periods): 
    must_in_variables = ['operationalDate', 'transferPoint', 'ownershipPercentage', 'netStandardVolume']
    ...
    # =============================================================================    
    #initial population
    #population = [list(np.random.randint(0,2,len(all_features_list))) for i in range(pop_size)]
    population = [list(np.random.choice([0,1], len(all_features_list), p=[0.9,0.1])) for i in range(pop_size)]
    if len(all_features_list) <1:
        results = must_in_variables
    if len(all_features_list) >=7:
...
def write_xgboost_models(transfer_points,fited_analytic_databases,best_features):
    ...
    for ix, transfer_point in enumerate(transfer_points):
        joblib.dump([xgboost_models[ix],best_features[ix]], base_directory + "//Modelos entrenados//XGBOOST_"+transfer_point+str(".dat"))
    return xgboost_models

# =============================================================================
# FUNCTIONS TO TRAIN, EVALUATE AND WRITE MODELS IF APPLIES
# =============================================================================
def train_test_arima_models(startTrainDate="2016-01-01",endTrainDate = "2019-12-31",periods=4,algorithmId=1):
...
def train_test_prophet_models(startTrainDate="2016-01-01",endTrainDate = "2019-12-31",periods=4,algorithmId=2):
    historic_ownership_values = get_historic_ownership_values_timeseries(startTrainDate,endTrainDate)
    transfer_points,analytic_databases = create_analytic_database_timeseries(algorithmId,historic_ownership_values)
...
def train_test_write_xgboost_models(startTrainDate="2016-01-01", endTrainDate = "2019-12-31",periods=4):
    ...
    best_features = list(map(lambda analytic_database: features_selection(analytic_database,pop_size=300,mutation_prob=0.05,elite_perc=0.1,pop_perc=0.6,generations=4,periods=4), tqdm(analytic_databases)))
    #Insertar reversión a CAÑO-LIMÓN
    for ix in range(len(transfer_points)):
        if transfer_points[ix] == "CAÑO_LIMON_OCLC" and 'FIELD-CAÑO LIMON - COVEÑAS-ARAGUANEY -BANADIA OBC' not in best_features[ix]:
            best_features[ix].append('FIELD-CAÑO LIMON - COVEÑAS-ARAGUANEY -BANADIA OBC')
...
# =============================================================================
arima_transfer_points, arima_analytic_databases,arima_evaluation_tables = train_test_arima_models()
prophet_transfer_points, prophet_analytic_databases,prophet_evaluation_tables = train_test_prophet_models()
xgb_transfer_points,xgb_analytic_databases,xgb_fited_analytic_databases,xgb_best_features,xgb_evaluation_tables,xgboost_models = train_test_write_xgboost_models()
