...
if($existing){
    $api = Import-AzApiManagementApi -Context $context -SpecificationUrl $swaggerUrl -SpecificationFormat OpenApi -Path $apiPath -ApiId $existing.ApiId
}else{
$api = Import-AzApiManagementApi -Context $context -SpecificationUrl $swaggerUrl -SpecificationFormat OpenApi -Path $apiPath
}

Set-AzApiManagementApi -Context $context -ServiceUrl $apiServiceUrl -ApiId $api.ApiId

Write-Output "Setting product as $productId for API"
