...
for %%i in (%platforms%) do (
	...
	for /f "tokens=1,2 delims= " %%a in ("!int!") do (
		...
		echo starting "%%i" dotnet test "%currentDir%%app%.csproj" -f "net461" -c "%config%" -a "%currentDir%bin\%config%" -v "n" --logger "trx" -r ".\TestResults" --filter !filter! --settings "%currentDir%%app%.Remote.%%a.%%b.runsettings" --no-restore --no-build
		start "%%i" dotnet test "%currentDir%%app%.csproj" -f "net461" -c "%config%" -a "%currentDir%bin\%config%" -v "n" --logger "trx" -r ".\TestResults" --filter !filter! --settings "%currentDir%%app%.Remote.%%a.%%b.runsettings" --no-restore --no-build
	)
)
goto end
...
for %%i in (%input%) do (
	...
	for /f "tokens=1,2 delims= " %%a in ("!int!") do (
		...
		REM /B /NEWWINDOW
		start "%%i" /B dotnet test "%currentDir%%app%.csproj" -f "net461" -c "%config%" -a "./" --no-build --logger "trx" -r "./TestResults" --filter !filter! --settings "%currentDir%%app%.runsettings" -- appSettings.browser=RemoteWebDriver appSettings.DriverCapabilities=CloudProvider appSettings.CrossBrowserEnvironment=%%a%%b
	)
)
goto end
