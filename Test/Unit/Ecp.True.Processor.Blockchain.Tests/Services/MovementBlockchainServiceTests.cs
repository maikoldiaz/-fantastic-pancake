// --------------------------------------------------------------------------------------------------------------------
// <copyright file="MovementBlockchainServiceTests.cs" company="Microsoft">
...
namespace Ecp.True.Processor.Blockchain.Tests
{
    ...
    /// </summary>
    [TestClass]
    public class MovementBlockchainServiceTests
    {
        /// <summary>
        /// The mock logger.
        /// </summary>
        private Mock<ITrueLogger<MovementBlockchainService>> mockLogger;
        ...
        /// </summary>
        private MovementBlockchainService blockchainMovementService;
        ...
        {
            this.mockLogger = new Mock<ITrueLogger<MovementBlockchainService>>();
            ...
            this.mockUnitOfWork.Setup(m => m.MovementRepository).Returns(this.mockMovementRepoMock.Object);

            this.blockchainMovementService = new MovementBlockchainService(
        ...
        /// <summary>
        /// Blockchains the movement service insert should register asynchronous.
        /// </summary>
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Insert_Should_RegisterAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Insert", false, Entities.Core.StatusType.PROCESSING);
            ...
            this.mockMovementRepository.Setup(r => r.SingleOrDefaultAsync(It.IsAny<Expression<Func<Movement, bool>>>(), It.IsAny<string[]>())).ReturnsAsync(movement);
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepoMock.Setup(a => a.GetLatestBlockchainMovementAsync(It.IsAny<string>())).ReturnsAsync((Movement)null);
            ...
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallMethodAsync<bool>(It.IsAny<string>(), It.IsAny<string>(), "HasOwner", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(false);
            ...
            await this.blockchainMovementService.RegisterAsync(movementTransactionId).ConfigureAwait(false);

            this.mockMovementRepository.Verify(r => r.SingleOrDefaultAsync(It.IsAny<Expression<Func<Movement, bool>>>(), It.IsAny<string[]>()), Times.Once);
            this.mockMovementRepository.Verify(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>()), Times.Once);
        ...
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Insert_Should_Register_BridgeAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Insert", false, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
            ...
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepoMock.Setup(a => a.GetLatestBlockchainMovementAsync(It.IsAny<string>())).ReturnsAsync(new Movement { EventType = EventType.Insert.ToString("G"), NetStandardVolume = 10.00M, BlockNumber = "0x908" });
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallSmartContractMethodAsync<ContractMetadataStruct>(It.IsAny<string>(), It.IsAny<string>(), "GetLatest", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(contractMetadataStruct);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallMethodAsync<bool>(It.IsAny<string>(), It.IsAny<string>(), "HasOwner", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(false);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.ExecuteTransactionOnSmartContractMethodAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, object>>())).ReturnsAsync(transactionReceipt);
        ...
        /// <summary>
        /// Blockchains the movement service update should register asynchronous.
        /// </summary>
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Update_Should_RegisterAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Update", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
            {
                ContractAddress = "ContractAddress",
                ContractAbi = "ContractAbi",
            ...
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepoMock.Setup(a => a.GetLatestBlockchainMovementAsync(It.IsAny<string>())).ReturnsAsync(new Movement { EventType = EventType.Insert.ToString("G"), NetStandardVolume = 10.00M, BlockNumber = "908" });
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallSmartContractMethodAsync<ContractMetadataStruct>(It.IsAny<string>(), It.IsAny<string>(), "GetLatest", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(contractMetadataStruct);
        ...
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Update_Should_Register_BridgeAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Update", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
        ...
        /// <summary>
        /// Blockchains the movement service delete should register asynchronous.
        /// </summary>
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Delete_Should_RegisterAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Delete", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
            {
                ContractAddress = "ContractAddress",
                ContractAbi = "ContractAbi",
        ...
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_Delete_Should_Register_BridgeAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Delete", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
        ...
        /// <summary>
        /// Blockchains the movement service if no movement exits should not register asynchronous.
        /// </summary>
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_If_No_Movement_Exits_Should_NotRegisterAsync()
        {
            var movementTransactionId = 1;
            var contractMetadataStruct = new ContractMetadataStruct
            {
            ...
            this.mockMovementRepository.Setup(r => r.SingleOrDefaultAsync(It.IsAny<Expression<Func<Movement, bool>>>(), It.IsAny<string[]>())).ReturnsAsync(() => null);
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(0);
            this.mockMovementRepoMock.Setup(a => a.GetLatestBlockchainMovementAsync(It.IsAny<string>())).ReturnsAsync((Movement)null);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallSmartContractMethodAsync<ContractMetadataStruct>(It.IsAny<string>(), It.IsAny<string>(), "GetLatest", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(contractMetadataStruct);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.ExecuteTransactionOnSmartContractMethodAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, object>>())).ReturnsAsync(transactionReceipt);
            ...
            this.mockMovementRepository.Verify(r => r.SingleOrDefaultAsync(It.IsAny<Expression<Func<Movement, bool>>>(), It.IsAny<string[]>()), Times.Once);
            this.mockMovementRepository.Verify(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>()), Times.Never);
        ...
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Unknown", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
            {
                ContractAddress = "Address",
            };
        ...
        /// <summary>
        /// Blockchains the movement service if number of unregistered movements is not zero should not register asynchronous.
        /// </summary>
        /// <returns>The task.</returns>
        [TestMethod]
        public async Task BlockchainMovementService_If_NumOfUnregisteredMovements_Is_Not_Zero_Should_NotRegisterAsync()
        {
            var movementTransactionId = 1;
            var (movement, owners) = this.GetMovement("Update", true, Entities.Core.StatusType.PROCESSING);
            var contractMetadataStruct = new ContractMetadataStruct
            ...
            this.mockMovementRepository.Setup(r => r.SingleOrDefaultAsync(It.IsAny<Expression<Func<Movement, bool>>>(), It.IsAny<string[]>())).ReturnsAsync(movement);
            this.mockMovementRepository.Setup(r => r.GetCountAsync(It.IsAny<Expression<Func<Movement, bool>>>())).ReturnsAsync(1);
            this.mockMovementRepoMock.Setup(a => a.GetLatestBlockchainMovementAsync(It.IsAny<string>())).ReturnsAsync((Movement)null);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.CallSmartContractMethodAsync<ContractMetadataStruct>(It.IsAny<string>(), It.IsAny<string>(), "GetLatest", It.IsAny<Dictionary<string, object>>())).ReturnsAsync(contractMetadataStruct);
            this.mockAzureclientFactory.Setup(x => x.EthereumClient.ExecuteTransactionOnSmartContractMethodAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<Dictionary<string, object>>())).ReturnsAsync(transactionReceipt);
        ...
        public async Task BlockchainService_ShouldRegisterCriticalEvent_WhenSendToQueueFailsAsync()
        {
            ...
            await this.blockchainMovementService.RegisterAsync(entityId).ConfigureAwait(false);
            ...
            this.mockMovementRepository.VerifyAll();
        ...
        private (Movement, IEnumerable<Owner>) GetMovement(string eventType, bool hasPreviousBlockchainTransactionId, StatusType blockchainStatus)
        {
